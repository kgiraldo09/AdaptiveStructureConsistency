...
import numpy as np
from cut.stylegan_networks import StyleGAN2Discriminator, StyleGAN2Generator, TileStyleGAN2Discriminator
...
def get_filter(filt_size=3, dim = 2):
    if(filt_size == 1):
        a = np.array([1., ])
    elif(filt_size == 2):
        a = np.array([1., 1.])
...
class Downsample(nn.Module):
    def __init__(self, channels, pad_type='reflect', filt_size=3, stride=2, pad_off=0, dim=2):
        ...
        filt = get_filter(filt_size=self.filt_size, dim = dim)
        if dim == 2:
            self.register_buffer('filt', filt[None, None, :, :].repeat((self.channels, 1, 1, 1)))
        else:
            self.register_buffer('filt', filt[None, None, :, :, :].repeat((self.channels, 1, 1, 1, 1)))
        if dim == 2:
            self.pad = get_pad_layer(pad_type, dim)(self.pad_sizes)
        else:
            self.pad = get_pad_layer(pad_type, dim)(self.pad_sizes+[1, 1])
...
class Upsample(nn.Module):
    def __init__(self, channels, pad_type='repl', filt_size=4, stride=2, dim=2):
        ...
        self.channels = channels
        self.dim = dim
        filt = get_filter(filt_size=self.filt_size, dim=dim) * (stride**2)     
        if dim == 2:
            self.register_buffer('filt', filt[None, None, :, :].repeat((self.channels, 1, 1, 1)))
            self.pad = get_pad_layer(pad_type, dim)([1, 1, 1, 1])
        else:
            self.register_buffer('filt', filt[None, None, :, :, :].repeat((self.channels, 1, 1, 1, 1)))
            self.pad = get_pad_layer(pad_type, dim)([1, 1, 1, 1, 1, 1])
...
def get_pad_layer(pad_type, dim=2):
...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal',
    ...
    """
    net = None
    norm_layer = get_norm_layer(norm_type=norm, dim = opt.dim)
    if netG == 'resnet_9blocks':
        net = ResnetGenerator(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, no_antialias=no_antialias, no_antialias_up=no_antialias_up, n_blocks=9, opt=opt, dim = opt.dim)
    elif netG == 'resnet_6blocks':
        net = ResnetGenerator(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, no_antialias=no_antialias, no_antialias_up=no_antialias_up, n_blocks=6, opt=opt, dim = opt.dim)
    elif netG == 'resnet_4blocks':
        net = ResnetGenerator(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, no_antialias=no_antialias, no_antialias_up=no_antialias_up, n_blocks=4, opt=opt, dim = opt.dim)
    elif netG == 'unet_128':
...
def define_F(input_nc, netF, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02, no_antialias=False, gpu_ids=[], opt=None):
    ...
    if netF == 'global_pool':
        ...
        net = PatchSampleF(use_mlp=True, init_type=init_type, init_gain=init_gain, gpu_ids=gpu_ids, nc=opt.netF_nc, dim = opt.dim)
...
def define_D(input_nc, ndf, netD, n_layers_D=3, norm='batch', init_type='normal', init_gain=0.02, no_antialias=False, gpu_ids=[], opt=None):
    ...
    if netD == 'basic':  # default PatchGAN classifier
        net = NLayerDiscriminator(input_nc, ndf, n_layers=3, norm_layer=norm_layer, no_antialias=no_antialias, dim = opt.dim)
...
class PatchSampleF(nn.Module):
    def __init__(self, use_mlp=False, init_type='normal', init_gain=0.02, nc=256, gpu_ids=[], dim=2):
    ...
    def forward(self, feats, num_patches=64, patch_ids=None):
        ...
        for feat_id, feat in enumerate(feats):
            if len(feat.shape) == 4:
                ...
                B, H, W, Z = feat.shape[0], feat.shape[2], feat.shape[3], feat.shape[4]
                feat_reshape = feat.permute(0, 2, 3, 4, 1).flatten(1, 2)
            ...
            if num_patches == 0:
                x_sample = x_sample.permute(0, 2, 1).reshape([B, x_sample.shape[-1], H, W, Z])
...
class ResnetGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, padding_type='reflect', no_antialias=False, no_antialias_up=False, opt=None, dim=2):
        ...
        for i in range(n_downsampling):  # add downsampling layers
            ...
            if(no_antialias):
                if dim == 2:
                    ...
                    model += [nn.Con3d(ngf * mult, ngf * mult * 2, kernel_size=3, stride=2, padding=1, bias=use_bias),
                          norm_layer(ngf * mult * 2),
                          nn.ReLU(True)]
            else:
                if dim == 2:
        ...
        for i in range(n_blocks):       # add ResNet blocks
            ...
            model += [ResnetBlock(ngf * mult, padding_type=padding_type, norm_layer=norm_layer, use_dropout=use_dropout, use_bias=use_bias, dim_ = dim)]
        ...
        for i in range(n_downsampling):  # add upsampling layers
            ...
            if no_antialias_up:
                ...
                if dim == 2:
                    model += [Upsample(ngf * mult, dim=dim),
...
class ResnetBlock(nn.Module):
    ...
    def __init__(self, dim, padding_type, norm_layer, use_dropout, use_bias, dim_):
        ...
        self.conv_block = self.build_conv_block(dim, padding_type, norm_layer, use_dropout, use_bias, dim_)

    def build_conv_block(self, dim, padding_type, norm_layer, use_dropout, use_bias, dim_):
        ...
        conv_block += [curr_conv(dim, dim, kernel_size=3, padding=p, bias=use_bias), norm_layer(dim_), nn.ReLU(True)]
        if use_dropout:
        ...
        if padding_type == 'reflect':
            ...
            raise NotImplementedError('padding [%s] is not implemented' % padding_type)
        conv_block += [curr_conv(dim, dim, kernel_size=3, padding=p, bias=use_bias), norm_layer(dim_)]
...
class NLayerDiscriminator(nn.Module):
    ...
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d, no_antialias=False, dim=2):
        ...
        padw = 1
        if(no_antialias):
            sequence = [currconv(input_nc, ndf, kernel_size=kw, stride=2, padding=padw), nn.LeakyReLU(0.2, True)]
        else:
            sequence = [currconv(input_nc, ndf, kernel_size=kw, stride=1, padding=padw), nn.LeakyReLU(0.2, True), Downsample(ndf, dim=dim)]
        nf_mult = 1
        nf_mult_prev = 1
        for n in range(1, n_layers):  # gradually increase the number of filters
            ...
            if(no_antialias):
                sequence += [
                    currconv(ndf * nf_mult_prev, ndf * nf_mult, kernel_size=kw, stride=2, padding=padw, bias=use_bias),
                    norm_layer(ndf * nf_mult),
                    nn.LeakyReLU(0.2, True)
                ]
            else:
                sequence += [
                    currconv(ndf * nf_mult_prev, ndf * nf_mult, kernel_size=kw, stride=1, padding=padw, bias=use_bias),
                    norm_layer(ndf * nf_mult),
                    nn.LeakyReLU(0.2, True),
                    Downsample(ndf * nf_mult, dim = dim)]
        ...
        sequence += [currconv(ndf * nf_mult, 1, kernel_size=kw, stride=1, padding=padw)]  # output 1 channel prediction map
